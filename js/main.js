...
function setupSizeButtons() {
  ...
  if (btn && info) {
    btn.addEventListener("click", async () => {
      ...
      if (!src) {
        info.textContent = "No image source found.";
        return;
      }
  ...
  if (btnBoth && sizeResults) {
    btnBoth.addEventListener("click", async () => {
      ...
      sizeResults.textContent = `Left: ${humanFileSize(
        l
      )} | Right: ${humanFileSize(r)}`;
    });
  }
}
...
function setupDownloadButton() {
  ...
  btn.addEventListener("click", async () => {
    ...
    const picture = document.getElementById("picture-sample");
    const imgElement =
      picture.querySelector("img") || picture.querySelector("source");
    ...
    try {
      ...
      console.error(err);
      alert(
        "Download failed (maybe blocked locally). Try deploying the site or use a hosted image."
      );
    }
  });
}
